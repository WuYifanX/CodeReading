...
window._ = {
  ...
  //所以思路是首先试探forEach,然后是数组那么自己遍历,如果有each那么执行;最后是对象,然后处理对象
  each : function(obj, iterator, context) {
    var index = 0;
    try {
      if (obj.forEach) { //如果是数组,那么可能会有forEach原生函数,直接调用;
        obj.forEach(iterator, context);
      } else if (obj.length) { //如果是以前的版本,array可能没有forEach,那么自己遍历一遍
        for (var i=0; i<obj.length; i++) iterator.call(context, obj[i], i);
      } else if (obj.each) { //如果存在each函数,可能是以前版本的underscore的each函数
        obj.each(function(value) { iterator.call(context, value, index++); });
      } else {
        var i = 0;
        for (var key in obj) { //这里如果不限定hasOwnproperty,可能会出现原型链上面的属性;
          var value = obj[key], pair = [key, value];
          pair.key = key; //往list里面放属性之后会变 pair =[ 'key', 'value', key: 'key' ]
          pair.value = value; //
          iterator.call(context, pair, i++);//传入的第一个参数是这个对象,第二个参数是第几个参数;
        }
      }
    } catch(e) {
  ...
  all : function(obj, iterator, context) {
    ...
    var result = true;
    _.each(obj, function(value, index) {
      //这里的!!用来强制转化布尔值;如果是a = undefined; !!a === false;
